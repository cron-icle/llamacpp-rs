...
pub fn llguidance_build_tok_env(model: &LlamaModel) -> TokEnv {
    ...
    let tok_eos = {
        let eot = unsafe { llama_cpp_rs_sys::llama_vocab_eot(model.vocab_ptr()) };
        if eot == -1 {
...
unsafe extern "C" fn llg_name(
    _smpl: *const llama_cpp_rs_sys::llama_sampler,
) -> *const std::os::raw::c_char {
    c"llguidance".as_ptr()
}

unsafe extern "C" fn llg_accept(
    smpl: *mut llama_cpp_rs_sys::llama_sampler,
    token: llama_cpp_rs_sys::llama_token,
) {
...
unsafe extern "C" fn llg_apply(
    smpl: *mut llama_cpp_rs_sys::llama_sampler,
    cur_p: *mut llama_cpp_rs_sys::llama_token_data_array,
) {
    let ctx = unsafe { &mut *(*smpl).ctx.cast::<LlgContext>() };
...
unsafe extern "C" fn llg_reset(smpl: *mut llama_cpp_rs_sys::llama_sampler) {
    let ctx = unsafe { &mut *(*smpl).ctx.cast::<LlgContext>() };
    let _ = ctx.matcher.reset();
}

unsafe extern "C" fn llg_clone(
    smpl: *const llama_cpp_rs_sys::llama_sampler,
) -> *mut llama_cpp_rs_sys::llama_sampler {
    let ctx = unsafe { &*(*smpl).ctx.cast::<LlgContext>() };
    let new_ctx = Box::new(LlgContext {
        matcher: ctx.matcher.deep_clone(),
    });
    unsafe {
        llama_cpp_rs_sys::llama_sampler_init(
...
unsafe extern "C" fn llg_free(smpl: *mut llama_cpp_rs_sys::llama_sampler) {
...
static mut LLG_SAMPLER_I: llama_cpp_rs_sys::llama_sampler_i = llama_cpp_rs_sys::llama_sampler_i {
    name: Some(llg_name),
