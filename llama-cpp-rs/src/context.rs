...
pub struct LlamaContext<'a> {
    pub(crate) context: NonNull<llama_cpp_rs_sys::llama_context>,
...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        llama_context: NonNull<llama_cpp_rs_sys::llama_context>,
        embeddings_enabled: bool,
    ) -> Self {
        Self {
    ...
    pub fn n_batch(&self) -> u32 {
        unsafe { llama_cpp_rs_sys::llama_n_batch(self.context.as_ptr()) }
    }

    /// Gets the max number of physical tokens (hardware level) to decode in batch. Must be less than or equal to [`Self::n_batch`].
    #[must_use]
    pub fn n_ubatch(&self) -> u32 {
        unsafe { llama_cpp_rs_sys::llama_n_ubatch(self.context.as_ptr()) }
    }

    /// Gets the size of the context.
    #[must_use]
    pub fn n_ctx(&self) -> u32 {
        unsafe { llama_cpp_rs_sys::llama_n_ctx(self.context.as_ptr()) }
    }
    ...
    pub fn decode(&mut self, batch: &mut LlamaBatch) -> Result<(), DecodeError> {
        let result =
            unsafe { llama_cpp_rs_sys::llama_decode(self.context.as_ptr(), batch.llama_batch) };
    ...
    pub fn encode(&mut self, batch: &mut LlamaBatch) -> Result<(), EncodeError> {
        let result =
            unsafe { llama_cpp_rs_sys::llama_encode(self.context.as_ptr(), batch.llama_batch) };
    ...
    /// - If the current model had a pooling type of [`llama_cpp_rs_sys::LLAMA_POOLING_TYPE_NONE`]
    ...
    pub fn embeddings_seq_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = llama_cpp_rs_sys::llama_get_embeddings_seq(self.context.as_ptr(), i);
    ...
    pub fn embeddings_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = llama_cpp_rs_sys::llama_get_embeddings_ith(self.context.as_ptr(), i);
    ...
    fn embeddings_out_len(&self) -> usize {
        let pooling = unsafe { llama_cpp_rs_sys::llama_pooling_type(self.context.as_ptr()) };
        if pooling == llama_cpp_rs_sys::LLAMA_POOLING_TYPE_RANK {
    ...
    pub fn get_logits(&self) -> &[f32] {
        let data = unsafe { llama_cpp_rs_sys::llama_get_logits(self.context.as_ptr()) };
    ...
    pub fn get_logits_ith(&self, i: i32) -> &[f32] {
        ...
        let data = unsafe { llama_cpp_rs_sys::llama_get_logits_ith(self.context.as_ptr(), i) };
        let len = usize::try_from(self.model.n_vocab()).expect("n_vocab does not fit into a usize");
    ...
    pub fn reset_timings(&mut self) {
        unsafe { llama_cpp_rs_sys::llama_perf_context_reset(self.context.as_ptr()) }
    }

    /// Returns the timings for the context.
    pub fn timings(&mut self) -> LlamaTimings {
        let timings = unsafe { llama_cpp_rs_sys::llama_perf_context(self.context.as_ptr()) };
    ...
    pub fn lora_adapter_set(
        ...
        let err_code = unsafe {
            llama_cpp_rs_sys::llama_set_adapters_lora(
                self.context.as_ptr(),
                adapters.as_mut_ptr(),
                1,
                scales.as_mut_ptr(),
    ...
    pub fn sampled_token_ith(&self, i: i32) -> Option<LlamaToken> {
        let token =
            unsafe { llama_cpp_rs_sys::llama_get_sampled_token_ith(self.context.as_ptr(), i) };
    ...
    pub fn print_memory_breakdown(&self) {
        unsafe { llama_cpp_rs_sys::llama_rs_memory_breakdown_print(self.context.as_ptr()) }
    }
}

impl Drop for LlamaContext<'_> {
    fn drop(&mut self) {
        unsafe { llama_cpp_rs_sys::llama_free(self.context.as_ptr()) }
    }
}
