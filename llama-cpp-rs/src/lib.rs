...
#[cfg(feature = "common")]
pub(crate) fn status_is_ok(status: llama_cpp_rs_sys::llama_rs_status) -> bool {
    status == llama_cpp_rs_sys::LLAMA_RS_STATUS_OK
}
...
/// ```
/// # use llama_cpp_rs::llama_time_us;
...
/// ```
/// # use llama_cpp_rs::mlock_supported;
...
pub fn json_schema_to_grammar(schema_json: &str) -> Result<String> {
    ...
    let rc = unsafe {
        llama_cpp_rs_sys::llama_rs_json_schema_to_grammar(schema_cstr.as_ptr(), false, &mut out)
    };
    ...
    unsafe { llama_cpp_rs_sys::llama_rs_string_free(out) };
    result
}
...
pub fn ggml_time_us() -> i64 {
    unsafe { llama_cpp_rs_sys::ggml_time_us() }
}

/// checks if mlock is supported
///
/// ```
/// # use llama_cpp_rs::llama_supports_mlock;
///
...
pub fn list_llama_ggml_backend_devices() -> Vec<LlamaBackendDevice> {
    ...
    for i in 0..unsafe { llama_cpp_rs_sys::ggml_backend_dev_count() } {
        ...
        let props = unsafe {
            ...
            llama_cpp_rs_sys::ggml_backend_dev_get_props(dev, &raw mut props);
            props
        };
        let name = cstr_to_string(props.name);
        let description = cstr_to_string(props.description);
        let backend = unsafe { llama_cpp_rs_sys::ggml_backend_dev_backend_reg(dev) };
        let backend_name = unsafe { llama_cpp_rs_sys::ggml_backend_reg_name(backend) };
        ...
        let device_type = match props.type_ {
            llama_cpp_rs_sys::GGML_BACKEND_DEVICE_TYPE_CPU => LlamaBackendDeviceType::Cpu,
            llama_cpp_rs_sys::GGML_BACKEND_DEVICE_TYPE_ACCEL => LlamaBackendDeviceType::Accelerator,
            llama_cpp_rs_sys::GGML_BACKEND_DEVICE_TYPE_GPU => LlamaBackendDeviceType::Gpu,
            llama_cpp_rs_sys::GGML_BACKEND_DEVICE_TYPE_IGPU => LlamaBackendDeviceType::IntegratedGpu,
...
extern "C" fn logs_to_trace(
    level: llama_cpp_rs_sys::ggml_log_level,
    ...
    if level == llama_cpp_rs_sys::GGML_LOG_LEVEL_CONT {
...
pub fn send_logs_to_tracing(options: LogOptions) {
    ...
    unsafe {
        ...
        llama_cpp_rs_sys::llama_log_set(Some(logs_to_trace), llama_heap_state as *mut _);
        llama_cpp_rs_sys::ggml_log_set(Some(logs_to_trace), ggml_heap_state as *mut _);
    }
}
