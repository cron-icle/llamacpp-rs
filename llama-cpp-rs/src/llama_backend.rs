...
use llama_cpp_rs_sys::ggml_log_level;
...
impl LlamaBackend {
    ...
    ///# use llama_cpp_rs::LlamaCppError;
    ///# use std::error::Error;
    ///
    ...
    pub fn init() -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { llama_cpp_rs_sys::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    ///# use llama_cpp_rs::llama_backend::LlamaBackend;
    ///# use std::error::Error;
    ///# use llama_cpp_rs::llama_backend::NumaStrategy;
    ///
    ///# fn main() -> Result<(), Box<dyn Error>> {
    ///
    ...
    pub fn init_numa(strategy: NumaStrategy) -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            llama_cpp_rs_sys::llama_numa_init(llama_cpp_rs_sys::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            llama_cpp_rs_sys::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub llama_cpp_rs_sys::ggml_numa_strategy,
);

impl TryFrom<llama_cpp_rs_sys::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: llama_cpp_rs_sys::ggml_numa_strategy) -> Result<Self, Self::Error> {
        match value {
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            llama_cpp_rs_sys::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for llama_cpp_rs_sys::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => llama_cpp_rs_sys::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { llama_cpp_rs_sys::llama_backend_free() }
    }
}
...
pub fn load_backends_from_path(path: &std::path::Path) {
    ...
    unsafe { llama_cpp_rs_sys::ggml_backend_load_all_from_path(s.as_ptr()) }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = llama_cpp_rs_sys::ggml_numa_strategy::from(*numa);
