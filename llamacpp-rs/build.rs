...
fn main() {
    ...
    let llama_src = Path::new(&manifest_dir).join("vendor/llama.cpp");
    ...
    println!("cargo:rerun-if-changed=ffi/wrapper_common.h");
    println!("cargo:rerun-if-changed=ffi/wrapper_common.cpp");
    println!("cargo:rerun-if-changed=ffi/wrapper_utils.h");
    println!("cargo:rerun-if-changed=ffi/wrapper_mtmd.h");
    ...
    if cfg!(feature = "common") {
        ...
        common_wrapper_build.compile("llama_cpp_sys_common_wrapper");
    }
    ...
    if cfg!(feature = "dynamic-backends") {
        ...
        // This crate's own compiled code reads this back via
        // `option_env!("GGML_BACKENDS_DIR")` (see `llama_backend::BACKENDS_DIR`).
        // The `DEP_LLAMA_BACKENDS_DIR` mechanism above is only propagated to
        // *downstream* crates' build scripts, not back to this crate's own
        // compilation, so it must be emitted directly here too.
        println!(
